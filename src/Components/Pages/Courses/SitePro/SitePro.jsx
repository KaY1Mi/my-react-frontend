...
import pro from '../../../../Image/webp/Course/SitePro/SitePro_main.png';
    ...
    <div className="bg-white">
      ...
      {showAuthModal && (
        <Modal onClose={() => setShowAuthModal(false)}>
          <div className="p-4">
            <h3 className="text-2xl font-bebas mb-4">{t.auth_required_title || "Требуется авторизация"}</h3>
            <p className="font-manrope mb-6">
              {t.auth_required_message || "Для оформления курса необходимо войти в систему или зарегистрироваться"}
            </p>
      ...
      <section className="grid grid-cols-2 gap-5 mx-2.5 md:grid-cols-4 md:gap-5 xl:grid-cols-10 xl:h-screen my-5">
          ...
          >
            <div>
              <p className="text-neutal-grey text-sm font-manrope md:text-xl">
                {t[`sitePro_card_${cardNum === 1 ? 'one' : cardNum === 2 ? 'two' : cardNum === 3 ? 'three' : 'four'}_lesson`]}
              </p>
              <h3 className="text-neutal-blue text-2xl font-bebas md:text-4xl">
                {t[`sitePro_card_${cardNum === 1 ? 'one' : cardNum === 2 ? 'two' : cardNum === 3 ? 'three' : 'four'}_work`]}
              </h3>
            </div>
        ...
        <div className="hidden xl:col-span-2 xl:block stat-item stat-top">
          <p className="text-xl font-manrope font-light">{t.sitePro_section1_p_two}</p>
        </div>
      </section>
      ...
      <section className="grid grid-cols-2 gap-5 mx-2.5 md:grid-cols-4 md:gap-5 xl:grid-cols-10 my-5">
        ...
        <div className="stat-item stat-bottom order-1 col-span-full px-2.5 py-5 bg-neutal-blue rounded-[10px] xl:row-start-2 xl:col-span-4 xl:order-2">
          <h3 className="font-bebas text-black text-6xl py-4 border-b-2 border-black">
            {t.sitePro_card_h3_one}
          </h3>
          <ul className="list-disc list-inside space-y-2.5 pt-5">
            {[1, 2, 3].map((item) => (
              <li key={item} className="font-manrope text-base text-black">
                {t[`sitePro_card_list_${item === 1 ? 'one' : item === 2 ? 'two' : 'three'}`]}
              </li>
            ))}
          </ul>
          <h4 className="text-2xl font-bebas text-black pt-7">{t.sitePro_card_h3_two}</h4>
          <ul className="list-disc list-inside space-y-2.5 pt-5">
            {[1, 2, 3, 4].map((item) => (
              <li key={item} className="font-manrope text-base text-black">
                {t[`sitePro_card_list_${item === 1 ? 'one_one' : item === 2 ? 'two_two' : item === 3 ? 'three_three' : 'four_four'}`]}
              </li>
            ))}
          </ul>
          <h5 className="font-bebas font-medium text-2xl pt-7">{t.sitePro_card_h3_three}</h5>
        </div>
            ...
            <ul className="list-disc list-inside space-y-2.5 pt-5">
              {[1, 2, 3, 4].map((item) => (
                <li key={item} className="font-manrope text-base text-white font-light">
                  {t[`sitePro_list_${item}`]}
                </li>
              ))}
            </ul>
            <h4 className="font-bebas text-6xl text-neutal-blue pt-12">59 900 ₽</h4>
          </div>
          <button 
        ...
        <div className="stat-item stat-top order-3 col-span-full xl:col-span-3 xl:row-start-2 xl:order-3">
          <div className="px-2.5 py-5 bg-neutal-black rounded-[10px]">
            ...
            <ul className="list-disc list-inside space-y-2.5 pt-5">
              {[1, 2, 3, 4].map((item) => (
                <li key={item} className="font-manrope text-base text-white font-light">
                  {t[`sitePro_list_${item}_${item}`]}
                </li>
              ))}
            </ul>
            <h4 className="font-bebas text-6xl text-neutal-blue pt-12">5 974 ₽</h4>
          </div>
          <button 
